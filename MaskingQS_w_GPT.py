...
answers = pd.DataFrame(columns=['GPT-4 Answer', 'Answer'])
                       
def mask_tokens(prompt):
    tokens = prompt.split()
    masked_tokens = tokens[:] 
    num_to_mask =int(round((1*len(tokens)))) 
...
for index, row  in df.iterrows(): 
            ...
            question = question + " " + options + " Please answer with only one letter, the correct answer option, nothing else."
            ...
            if answer: 
                ...
                with open("Answers(GPT-4 MCQ 100% Masking).csv", 'a', newline='', encoding='utf-8') as f:
